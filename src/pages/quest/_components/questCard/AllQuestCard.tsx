    ...
    <div className="relative rounded-2xl overflow-hidden shadow-lg bg-black/80 text-white w-full">
      ...
      <div className="fontClass absolute top-0 left-0 w-full p-3 flex justify-between items-start">
        <div className="flex items-center gap-2">
          <img
            src={quest.authorAvatar}
            alt={quest.authorName}
            className="w-10 h-10 rounded-full object-cover"
          />
          <div>
            <p className="text-lg font-semibold">{quest.authorName}</p>
            <p className="text-md font-medium text-[#EFEFF0]">@{quest.authorUsername}</p>
          </div>
        </div>
...
<path d="M13 6.34281V2L21 9.59992L13 17.1998V12.857C13 12.857 1 11.8462 1 18C1 5.69139 13 6.34281 13 6.34281Z" stroke="white" stroke-width="1.2" stroke-miterlimit="10"/>
</svg>
</div>
      ...
      <div className="fontClass absolute bottom-0 left-0 w-full">
        ...
        <p className="font-semibold text-2xl text-white text-right">${quest.price}</p>
        </div>
        <div className="flex justify-between items-center mt-2 lg:mt-3">
          ...
          <div className="text-right">
            ...
            <p className="text-xs font-medium text-white">{quest.date}</p>
          </div>
        </div>
        </div>
      </div>
