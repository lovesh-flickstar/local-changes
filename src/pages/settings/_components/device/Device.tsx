...
const DeviceIcon2  = () => (
  <svg width="50" height="50" viewBox="0 0 73 50" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div className="flex items-start justify-between py-2 border-b border-white/10">
    <div className="flex items-start gap-3">
      ...
      <div>
        ...
        <p className="text-xs text-white/60">IP: 192.168.1.1 • New York, USA</p>
      </div>
    </div>
...
export const DevicesActivity = () => {
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 text-white">
      <div className="flex flex-col items-center space-y-4 mb-6">
      <button className="bg-blue-950/50 px-3 py-5 rounded-full">
        <DeviceIcon />
      </button>
        <h1 className="text-xl font-semibold">Devices Activity</h1>
        <p className="text-sm text-white/60 text-center max-w-md">
          “Devices Recent Activity” displays a list of devices that have accessed your
          account, showing details like device type, location, and login time. This
          helps monitor unauthorized access and enhance security.
        </p>
      </div>

      {/* Online device block */}
      <div className="bg-white/10 p-4 rounded-xl mb-6">
        <DeviceItem time="" isOnline />
        <div className="text-red-500 text-sm mt-4 cursor-pointer hover:underline">
          Terminate all the sessions
        </div>
      </div>

      {/* History */}
      <h2 className="text-lg font-medium mb-4">Devices Recent Activity</h2>
      <div className="space-y-3">
        {devices.map((d, idx) => (
          <DeviceItem key={idx} time={d.time} />
        ))}
      </div>
    </div>
  );
